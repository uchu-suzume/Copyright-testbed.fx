...
uniform int cLayer_Select <
    ...
    ui_tooltip = "The image/texture you'd like to use.  ";//Addending spaces to inhibit unnatural line breaks of tooltip.
    ui_type = "combo";
    ui_items = 
               "FFXIV\0"
               "FFXIV Nalukai\0"
               ...
               "Custom\0"
               ;
    ui_bind = "Copyright_Texture_Source";
> = 0;
...
sampler Copyright_Sampler_Gauss_Out
{ 
    Texture = Copyright_Texture_Gauss_Out;
};

sampler Copyright_Sampler_CAb
{ 
    Texture = Copyright_Texture_CAb;
};
...
sampler Copyright_Sampler_CAb_Gauss_Out
{ 
    Texture = Copyright_Texture_CAb_Gauss_Out;
};
...
float4 PS_cLayer_Gauss_H(in float4 pos : SV_Position, in float2 texCoord : TEXCOORD) : COLOR  {
        ...
        float4 color = tex2D(Copyright_Sampler_Gauss_Out, texCoord);
        switch(GaussianBlurRadius)
        {
             default:
                 ...
                 for(int i = 1; i < 2; ++i)
                 {
                 color += tex2Dlod(Copyright_Sampler_Gauss_Out, float4(texCoord + float2(sampleOffsets[i] * (GaussWeight * (GaussWeightH+ 0.5)) * PIXEL_SIZE.x, 0.0), 0.0, 0.0)) * sampleWeights[i];
                 color += tex2Dlod(Copyright_Sampler_Gauss_Out, float4(texCoord - float2(sampleOffsets[i] * (GaussWeight * (GaussWeightH+ 0.5)) * PIXEL_SIZE.x, 0.0), 0.0, 0.0)) * sampleWeights[i];
                 }
                 break;
            case 1:
...
float4 PS_cLayer_CAb(in float4 pos : SV_Position, in float2 texCoord : TEXCOORD) : COLOR  {

        const float2 CAb_Shift = cLayer_CAb_Shift * 0.05;
        const float3 SumUV_R = mul (mul (mul (mulUV, positionMatrix(PosX + CAb_Shift.x, PosY + CAb_Shift.y)), rotateMatrix(cLayer_Rotate)), scaleMatrix(ScaleX, ScaleY));
        const float3 SumUV_B = mul (mul (mul (mulUV, positionMatrix(PosX - CAb_Shift.x, PosY - CAb_Shift.y)), rotateMatrix(cLayer_Rotate)), scaleMatrix(ScaleX , ScaleY));

        float4 colorR = tex2D(Copyright_Sampler, SumUV_R.rg + pivot.rg) * all(SumUV_R + pivot == saturate(SumUV_R + pivot));
        colorR.rgb = float3(saturate(1 - cLayer_CAb_Hue.x), saturate(0.5 - (cLayer_CAb_Hue.y * 2)), saturate(cLayer_CAb_Hue.y - 0.5));
        colorR.a = colorR.a;

        float4 colorB = tex2D(Copyright_Sampler, SumUV_B.rg + pivot.rg) * all(SumUV_B + pivot == saturate(SumUV_B + pivot));
        colorB.rgb = float3(saturate(cLayer_CAb_Hue.x + 1), saturate(cLayer_CAb_Hue.y - 0.5), saturate(1 - cLayer_CAb_Hue.x));
        colorB.a = colorB.a;
...
float4 PS_cLayer_CAb_Gauss_H(in float4 pos : SV_Position, in float2 texCoord : TEXCOORD) : COLOR  {
        ...
        float4 color = tex2D(Copyright_Sampler_CAb_Gauss_Out, texCoord);
        const float sampleOffsets[6] = { 0.0, 1.4584295168, 3.40398480678, 5.3518057801, 7.302940716, 9.2581597095 };
        const float sampleWeights[6] = { 0.13298, 0.23227575, 0.1353261595, 0.0511557427, 0.01253922, 0.0019913644 };
        color *= sampleWeights[0];
        for(int i = 1; i < 6; ++i)
        {
        color += tex2Dlod(Copyright_Sampler_CAb_Gauss_Out, float4(texCoord + float2(sampleOffsets[i] * cLayer_CAb_Blur * PIXEL_SIZE.x, 0.0), 0.0, 0.0)) * sampleWeights[i];
        color += tex2Dlod(Copyright_Sampler_CAb_Gauss_Out, float4(texCoord - float2(sampleOffsets[i] * cLayer_CAb_Blur * PIXEL_SIZE.x, 0.0), 0.0, 0.0)) * sampleWeights[i];
        }
        return color;
}
...
float4 PS_cLayer_CAb_Gauss_V(in float4 pos : SV_Position, in float2 texCoord : TEXCOORD) : COLOR  {
        ...
        float4 color = tex2D(Copyright_Sampler_CAb, texCoord);
        const float sampleOffsets[6] = { 0.0, 1.4584295168, 3.40398480678, 5.3518057801, 7.302940716, 9.2581597095 };
        const float sampleWeights[6] = { 0.13298, 0.23227575, 0.1353261595, 0.0511557427, 0.01253922, 0.0019913644 };
        color *= sampleWeights[0];
        for(int i = 1; i < 6; ++i)
...
void PS_cLayer(in float4 pos : SV_Position, float2 texCoord : TEXCOORD, out float4 passColor : SV_Target) {
    ...
    if (Depth < cLayer_Depth)
    {
        ...
        const float4 CAb_Out = tex2D(Copyright_Sampler_CAb_Gauss_Out, texCoord);
        ...
        GaussOut = lerp(GaussOut.rgb, Screen(GaussOut.rgb, CAb_Out.rgb), CAb_Out.a * cLayer_CAb_Strength);
        switch(cLayer_BlendMode_CAb)
        {
            default:
                break;
            case 1:
                GaussOut = lerp(GaussOut.rgb, ColorB(GaussOut.rgb, CAb_Out.rgb), CAb_Out.a * cLayer_CAb_Strength);
                break;
            case 2:
                GaussOut = lerp(GaussOut.rgb, GrainMerge(GaussOut.rgb, CAb_Out.rgb), CAb_Out.a * cLayer_CAb_Strength);
                break;
            case 3:
                GaussOut = lerp(GaussOut.rgb, float3(1, 1, 1), CAb_Out.a * cLayer_CAb_Strength);
                break;
            case 4:
                GaussOut = lerp(GaussOut.rgb, float3(0, 0, 0), CAb_Out.a * cLayer_CAb_Strength);
                break;
        }
...
technique Copyright< ui_label = "Copyright"; >
{
    ...
    {
        VertexShader = PostProcessVS;
        PixelShader = PS_cLayer_CAb;
        RenderTarget = Copyright_Texture_CAb;
    }
    pass pass3
    {
        VertexShader = PostProcessVS;
